...
export default function DonationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!nameRegex.test(formData.fullName.trim())) newErrors.fullName = lang === "mr" ? "नावात फक्त अक्षरे असावीत" : "Name should contain only letters";
    const mobileRegex = /^[0-9]{10}$/;
    if (!mobileRegex.test(formData.mobile)) newErrors.mobile = lang === "mr" ? "मोबाईल क्रमांक 10 अंकांचा असावा" : "Mobile number must be exactly 10 digits";
    const emailRegex = /^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.(com|in|net)$/;
    if (!emailRegex.test(formData.email)) newErrors.email = lang === "mr" ? "वैध ईमेल टाका (.com, .in किंवा .net)" : "Enter a valid email ending with .com, .in or .net";
    setErrors(newErrors);
  ...
  return (
    <main className="relative w-full overflow-hidden bg-[#d8c7a8] pt-[64px]">
      {/* ================= HERO SECTION ================= */}
      <motion.section 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        >
          <div className="flex items-center gap-10 md:gap-16">
            ...
            >
              ...
              >
                <Image src={activeTab === "donation" ? "/donation-orange.svg" : "/ऑनलाइन देणगी फॉर्म.svg"} alt="icon" fill priority className="object-contain" />
              </motion.div>
      ...
      <AnimatePresence mode="wait">
        {activeTab === "donation" && (
          <motion.section 
            ...
            exit={{ opacity: 0, y: -30 }}
            transition={{ duration: 0.5 }}
            className="bg-[#d8c7a8] px-4 pb-24"
          >
            <motion.div 
            ...
            >
              <motion.div 
                ...
                animate="visible"
              >
                <motion.h2 className="text-[28px] md:text-[36px] font-medium text-[#222]" variants={itemVariants}>
                  {t.formTitle}
                </motion.h2>
                <motion.p className="text-[#555] mt-6 text-[17px]" variants={itemVariants}>
                  {t.personalInfo}
                </motion.p>
              </motion.div>
              ...
              <form onSubmit={handleSubmit} className="mt-10">
                ...
                >
                  <motion.div variants={itemVariants}>
                    ...
                    />
                    {errors.fullName && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-red-500 text-sm mt-1">{errors.fullName}</motion.p>}
                  </motion.div>
                  ...
                  <motion.div variants={itemVariants}>
                    ...
                    <input 
                      ...
                      className="w-full mt-2 h-[42px] border border-[#bdbdbd] rounded-[8px] px-4 bg-transparent outline-none text-[14px] focus:border-[#C05621] transition-colors duration-300" 
                    />
                    {errors.mobile && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-red-500 text-sm mt-1">{errors.mobile}</motion.p>}
                  </motion.div>
                  ...
                  <motion.div variants={itemVariants}>
                    ...
                    <input 
                      ...
                      className="w-full mt-2 h-[42px] border border-[#bdbdbd] rounded-[8px] px-4 bg-transparent outline-none text-[14px] focus:border-[#C05621] transition-colors duration-300" 
                    />
                    {errors.email && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-red-500 text-sm mt-1">{errors.email}</motion.p>}
                  </motion.div>
                ...
                >
                  <h3 className="text-[22px] text-[#333] font-medium">{t.donationDetails}</h3>
                </motion.div>
                ...
                >
                  ...
                  <motion.div variants={itemVariants}>
                    ...
                    <select className="w-full mt-2 h-[42px] border border-[#bdbdbd] rounded-[8px] px-4 bg-transparent outline-none text-[14px] focus:border-[#C05621] transition-colors duration-300">
                ...
                >
                  <motion.button 
                    type="button" 
                    className="w-[120px] h-[44px] border border-[#444] rounded-[8px] text-[#333] text-[15px] flex items-center justify-center gap-2 bg-white"
                    whileHover={{ scale: 1.05, backgroundColor: "#f0f0f0" }}
                    whileTap={{ scale: 0.95 }}
                  >
                    <div className="relative w-[18px] h-[18px]">
                      <Image src="/रद्द.svg" alt="icon" fill priority className="object-contain" />
                    </div>
                    <span>{t.cancel}</span>
                  </motion.button>
                  ...
                  <motion.button 
                    type="submit" 
                    className="w-[120px] h-[44px] bg-[#C05621] hover:bg-[#9f4b1c] rounded-[8px] text-white text-[15px] flex items-center justify-center gap-2 transition shadow-md disabled:opacity-50"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    disabled={isSubmitting}
                  >
                    <div className="relative w-[18px] h-[18px]">
                      <Image src="/देणगी.svg" alt="icon" fill priority className="object-contain" />
                    </div>
                    <span>{isSubmitting ? (lang === "mr" ? "प्रक्रिया..." : "Processing...") : t.donate}</span>
                  </motion.button>
                </motion.div>
              </form>
            </motion.div>
        ...
        {activeTab === "bank" && (
          ...
          >
            ...
            >
              ...
              <motion.div 
                className="mt-10 bg-[#f6dfd2] border-l-4 border-[#ff6a00] rounded-[10px] px-5 py-5 text-[14px] text-[#5a4638]"
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{ delay: 0.5 }}
                whileHover={{ scale: 1.02 }}
